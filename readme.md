...
**The Business Problem:** Total Service Failure (OTIF %) plummeted to **29.02%**, a **36%** deviation from the target.

**The Key Finding:** The failure is structural: **Inventory Control** is failing (**65.96%** LIFR), while **Warehouse Capacity** is sufficient (**96.59%** VOFR). The problem is a **Planning failure**.
...
| :--- | :--- | :--- |
| **Volume Fill Rate (VOFR %)** | Measures if the warehouse had enough *total capacity*. | **High VOFR (96.59%) = Capacity is sufficient.** |
| **Line-Item Fill Rate (LIFR %)** | Measures *how often* an SKU is stocked out. | **Low LIFR (65.96%) = Inventory Control Problem.** (Root Cause) |

SQL logic available in: [`supply_chain.sql`](supply_chain.sql)
...
> **Diagnostic Conclusion:** The **100%** planning failure is proven by **High VOFR (96.59%)** vs. **Low LIFR (65.96%)**. The problem is inventory control.
...
| Action | Owner | Goal |
| :--- | :--- | :--- |
| **Immediate Stock Review** | Supply Planning Team | Correct planning parameters for the **Top 5 failing SKUs**. |
| **LIFR % Stabilization** | Head of Supply Chain | Increase **Actual LIFR %** from **65.96%** to target $\approx 75\%$ (30 days). |
...
| Action | Owner | Goal |
| :--- | :--- | :--- |
| **Customer Engagement** | Sales & Account Management | Contact **Top 5 High-Risk Customers** to outline the corrective plan. |
| **Logistics Process Fix** | Distribution Manager | Reduce the **1-Day Delays** bucket by **50%** (reducing **8.3K** delayed orders). |
...
| :--- | :--- |
| **Email ID** | [gagancholkar@gmail.com](mailto:gagancholkar@gmail.com) |
